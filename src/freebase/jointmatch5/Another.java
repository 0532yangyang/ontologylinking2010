...
public class Another {
	...
	public static void testBing() {
		List<String[]> all = BingApi.getResult("Seminoles Bobby Bowden");
	}

	public static void anyGoldMappingAppearsInNonSenseJoin() throws IOException {
		String file_nonsense = Main.dir + "/sql2instance_union.debug";
		String file_gold = Main.dir + "/mycreateontology/gold_mapping";
		HashSet<String> golds = new HashSet<String>();
		{
			List<String[]> all = (new DelimitedReader(file_gold)).readAll();
			for (String[] l : all)
				golds.add(l[1]);
		}
		{
			DelimitedReader dr = new DelimitedReader(file_nonsense);
			String[] l;
			while ((l = dr.read()) != null) {
				if (golds.contains(l)) {
					D.p(l);
...
//		analyzeWidpairPatterns();
		anyGoldMappingAppearsInNonSenseJoin();

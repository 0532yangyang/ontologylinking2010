...
public class Step1_candidatemapping_nelltype_fbtype_count {
	...
	public static void subsetfin_freebase_type_sortMid() throws Exception {
		DelimitedReader dr = new DelimitedReader(
				Main.fin_enid_mid_wid_argname_otherarg_relation_label_sortbywid);
...
private static void loadType(String file){
	try{
		...
		while((line = dr.read())!=null){
			...
			//filter the type
			String type0 = line[1];
			if(type0.startsWith("/m/") || type0.startsWith("/common/") ||type0.startsWith("/base/")){
				continue;
			}
	...
	public static void mapping() throws Exception {
		loadType(Main.fout_freebase_type_sortMid_subset);
		...
		DelimitedWriter dw = new DelimitedWriter(Main.fout_candidatemapping_nelltype_fbtype_count);

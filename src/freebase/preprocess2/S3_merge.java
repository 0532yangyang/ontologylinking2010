...
public class S3_merge {
	...
	private static void getPreviousLargetsId() throws IOException {
		...
		for (int i = 0; i < Main.datas.length; i++) {
			...
			String fname = Main.fout_wexsplit_template.replace("$DNUM$", Main.datas[i]) + i;
	...
	private static void mergeAlignID(String suffix) throws IOException {
		BufferedWriter w = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(Main.file_parsed_wikisections
				+ suffix), "utf-8"));
		for (int i = 0; i < Main.datas.length; i++) {
			String fname = Main.fout_wexsplit_template.replace("$DNUM$", Main.datas[i]) + i;
			DelimitedReader r1 = new DelimitedReader(fname + ".ids");

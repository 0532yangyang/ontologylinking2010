...
public class WeightedClauses {
	...
	int updateOneStep() {
		...
		if (unitclause.size() == 0) {
			...
			for (int i = 0; i < literals.length; i = i + 2) {
				// This literal (and its negation) has already been assigned
				if (literal_values[i] != 0) {
					continue;
				}
				double w1 = 0, w2 = 0;
				for (int c : literals[i].getDims()) {
					if (unitclause.contains(c)) {
						w1 += cweights[c];
					}
				}
				for (int c : literals[i + 1].getDims()) {
					if (unitclause.contains(c)) {
						w2 += cweights[c];
					}
				}
	...
	void assignVariableValue(int literalId) {
...

		{
			...
			for (int cid : satClauses) {
				unknown_literal_num_clause[cid] = 0; // value of other variables
														// are not important any
														// more
			}
		}
		{
			// for the other literalId, its corresponding "unsat" clauses's
			// unknown variables are one less
			...
			for (int cid : satClauses) {
				...
				if (unknown_literal_num_clause[cid] == 1) { // this clause will
															// be unsat ever
					// to do what?
				}
			}
